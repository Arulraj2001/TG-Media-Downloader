...
class WorkerSignals(QObject):
    ...
    file_failed = Signal(str, int, str) # task_id, msg_id, error
    task_incomplete = Signal(str, int) # task_id, failed_count
...
class TelegramWorker(QThread):
    ...
    # DOWNLOADS
    # -------------------------------------------------------------------------
    async def _resolve_channel(self, channel_input):
        """Resolve an entity once and share it across simultaneous bulk tasks."""
        cache_key = str(channel_input).strip()
        cached = self._channel_cache.get(cache_key)
        if cached is not None:
            return cached

        lock = self._channel_locks.setdefault(cache_key, asyncio.Lock())
        async with lock:
            cached = self._channel_cache.get(cache_key)
            if cached is not None:
                return cached
            ...
            # Numeric IDs are used after the first resolution. Cache that alias
            # too, so resumed tasks do not make another Telegram API request.
            try:
                from telethon.utils import get_peer_id
                self._channel_cache[str(get_peer_id(channel))] = channel
            except Exception:
                pass
            return channel

    def _get_download_semaphore(self, requested_limit):
        """Return the worker-wide transfer limiter.

        Very high Telegram parallelism is unstable in practice. Eight keeps
        bulk downloads fast while avoiding connection/request storms.
        """
        safe_limit = max(1, min(int(requested_limit or 1), 8))
        if self._download_semaphore is None:
            self._download_concurrency = safe_limit
            self._download_semaphore = asyncio.Semaphore(safe_limit)
    ...
    async def _download_coro(self, channel_input, media_id, download_path, download_limit, max_speed_kb, is_paused, selected_message_ids, original_task_id=None):
        try:
            ...
            if selected_message_ids is not None:
                ...
                # Full-history scans are deliberately serialized. Bulk mode
                # starts one task per category, and concurrent unbounded scans
                # can make Telegram close or decline otherwise valid requests.
            ...
            failed_ids = await download_in_batches_headless(
                client=self.client,
                channel=channel,
                ...
                shared_semaphore=self._get_download_semaphore(download_limit),
            )

            if failed_ids:
                # Keep the task available for an explicit retry and persist
                # that state so a restart does not hide the failure.
                try:
                    task_list = load_active_tasks()
                    for saved_task in task_list:
                        same_channel = (
                            str(saved_task.get("channel_input")).replace("-100", "", 1)
                            == resolved_chan_id.replace("-100", "", 1)
                        )
                        if (
                            same_channel
                            and saved_task.get("media_id") == media_id
                            and saved_task.get("topic_id") == topic_id
                        ):
                            saved_task["paused"] = True
                            break
                    save_active_tasks(task_list)
                except Exception as persist_error:
                    print(f"Error persisting failed task state: {persist_error}")
                self.signals.task_incomplete.emit(task_id, len(failed_ids))
