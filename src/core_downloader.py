...
async def fetch_channel(client, channel_input):
    ...
    def dialog_matches(dialog):
        ...
        if (
            dialog_id == wanted
            or dialog_id.replace("-100", "", 1)
            == wanted.replace("-100", "", 1)
        ):
            return True

        title = str(getattr(dialog, "title", "") or "").strip().casefold()
        username = str(
        ...
        try:
            ...
            try:
                resume_offset = (
                    ...
                    if expected_filepath
                    and file_size
                    and 0 < existing_size < file_size
                    else 0
                )
                if resume_offset:
                    # Telethon's high-level download_media opens paths with
                    # "wb", which silently restarted partial files. Stream
                    # from the existing aligned offset and append instead.
                    await internal_progress(resume_offset, file_size)
                    with open(expected_filepath, "ab") as output:
                        ...
                        async for chunk in client.iter_download(
                            ...
                            output.write(chunk)
                            current += len(chunk)
                            await internal_progress(current, file_size)
                    file_path = expected_filepath
                    if os.path.getsize(file_path) < file_size:
                        raise RuntimeError(
                            f"Partial download ended at {os.path.getsize(file_path)} "
            ...
            if not file_path:
                raise RuntimeError("Telegram returned no file for this message")
            ...
            if attempt < max_retries - 1:
                ...
                # Do not mark a failed transfer as downloaded. Leaving it
                # incomplete lets a resume/retry attempt fetch it again.
                if complete_cb:
                    complete_cb(
                        message.id,
                        paused=False,
...
async def download_in_batches_headless(
    client,
    channel,
    messages,
    folder_name,
    ...
    """Download messages without multiplying concurrency across bulk tasks.

    ``shared_semaphore`` is supplied by the worker so a download limit of five
    means five transfers in total, not five transfers for every selected media
    category.
    """
    semaphore = shared_semaphore or asyncio.Semaphore(max(1, int(batch_size or 1)))
...
async def get_messages_by_type(client, channel, media_choice, min_id=None, max_id=None, limit=None, topic_id=None):
    ...
    if media_choice == 1:
        ...
        # The bulk UI describes this option as images + videos. Using no
        # filter fetched the entire chat history, including text-only posts.
        filter_type = InputMessagesFilterPhotoVideo()
    else:
        filter_type = None
    ...
    if media_choice == 3:
        # The bulk UI promises all files/documents. Previously this silently
        # discarded every non-PDF document.
        messages = [
            m for m in messages
            if m.document
            and not (m.document.mime_type or "").startswith("audio/")
        ]
    elif media_choice == 4:
        archive_types = {
            "application/zip",
        ...
        messages = [
            m for m in messages
            if m.document and (m.document.mime_type or "") in archive_types
        ]
    elif media_choice == 5:
        messages = [
            m for m in messages
