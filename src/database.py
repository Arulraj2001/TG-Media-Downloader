...
def init_db():
    ...
    try:
        cursor.execute("ALTER TABLE tasks ADD COLUMN topic_id INTEGER")
    except sqlite3.OperationalError:
        pass
...
def save_task_db(task):
    ...
    task_id = (
        f"{ch_key}_{topic_id}_{media_id}"
        if topic_id is not None
        else f"{ch_key}_{media_id}"
    )
    ...
    cursor.execute("""
    INSERT INTO tasks (
        task_id, channel_input, media_id, topic_id, paused, download_path,
    ...
    """, (
        task_id, ch, media_id, topic_id, 1 if task.get("paused") else 0,
