...
class DownloadCard(QWidget):
    ...
    def setup_ui(self, media_type):
        ...
        # badge
        badge_text = {1:"IMG",2:"VID",3:"DOC",4:"ZIP",5:"AUD",6:"ALL"}.get(media_type,"ALL")
        ...
        # row 2 – media pill
        pr = QHBoxLayout()
        pill_label = {1:"Images",2:"Videos",3:"Documents",
                      4:"Archives",5:"Audio",6:"All media"}.get(media_type,"All media")
        ...
        self.batch_progress_bar.setMaximum(1000)
        initial_pct = (
            int(self.completed * 1000 / self.total_items)
            if self.total_items else 0
        )
        self.batch_progress_bar.setValue(initial_pct)
        ...
        self.lbl_verify_status.setStyleSheet("color: #9562E3; font-weight: bold;")
        ...
        self.btn_verify = QPushButton("Check files")
        ...
        self.btn_reselect = QPushButton("Change files")
        self.btn_reselect.setObjectName("CardButton")
        self.btn_reselect.setToolTip("Add or remove files from this download")
        ...
        self.btn_up.setToolTip("Move this download up")

        self.btn_down = QPushButton("DN")
        self.btn_down.setFixedWidth(38)
        self.btn_down.setObjectName("CardButtonCompact")
        self.btn_down.setToolTip("Move this download down")
        ...
        tools_row.addWidget(self.btn_verify)
        tools_row.addWidget(self.btn_expand)
        tools_row.addStretch()
        rc.addLayout(tools_row)
        ...
        hdr = QLabel("FILES IN THIS DOWNLOAD")
    ...
    def populate_files(self):
        ...
        if len(self.files_metadata) > max_visible:
            ...
            lbl_more = QLabel(f"{more_count} more files in this queue")
    ...
    def update_file_progress(self, msg_id, current_bytes, total_bytes, speed_str):
        # Keep state for every parallel file instead of letting the latest
        # signal overwrite the speed reported by the whole card.
        import re
        ...
        self.file_transfer_stats[msg_id] = {
            "current": max(0, current_bytes),
            "total": max(0, total_bytes),
            "speed_kb": speed_kb,
        }
        self._refresh_live_transfer_status(msg_id)
    ...
    def mark_file_completed(self, msg_id):
        ...
        for metadata in self.files_metadata:
            if metadata["id"] == msg_id:
                metadata["completed"] = True
                break
        self.failed_files.discard(msg_id)
        self.file_transfer_stats.pop(msg_id, None)
        self._refresh_live_transfer_status()

    def mark_file_failed(self, msg_id, error):
        self.failed_files.add(msg_id)
        self.file_transfer_stats.pop(msg_id, None)
        if msg_id in self.file_rows:
            self.file_rows[msg_id].set_failed()
        self._refresh_live_transfer_status()
        short_error = (error or "Download failed").replace("\n", " ")[:90]
        self.lbl_status_text.setText(f"Retry needed: {short_error}")

    def set_incomplete(self, failed_count):
        self.is_paused = True
        self.last_speed_val = 0
        self.file_transfer_stats.clear()
        self.btn_pause.setEnabled(True)
        self.btn_pause.setText("Retry failed")
        self.lbl_status_text.setText(
            f"{failed_count} file{'s' if failed_count != 1 else ''} failed • Retry available"
        )
        self.lbl_status_text.setProperty("state", "paused")
        self.batch_progress_bar.setProperty("state", "paused")
        self.lbl_status_text.style().unpolish(self.lbl_status_text)
    ...
    def _refresh_live_transfer_status(self, latest_msg_id=None):
        self.last_speed_val = sum(
            item["speed_kb"] for item in self.file_transfer_stats.values()
        )
        self._refresh_aggregate_progress()

        if not self.file_transfer_stats:
            if not self.is_paused and self.completed < self.total_items:
                self.lbl_status_text.setText("Finishing...")
            return

        if self.last_speed_val >= 1024 * 1024:
            speed_text = f"{self.last_speed_val / (1024 * 1024):.2f} GB/s"
        elif self.last_speed_val >= 1024:
            speed_text = f"{self.last_speed_val / 1024:.1f} MB/s"
        else:
            speed_text = f"{self.last_speed_val:.0f} KB/s"

        latest = self.file_transfer_stats.get(latest_msg_id)
        if latest is None:
            latest = next(iter(self.file_transfer_stats.values()))
        file_pct = (
            int(latest["current"] * 100 / latest["total"])
            if latest["total"] > 0 else 0
        )
        active_count = len(self.file_transfer_stats)
        self.lbl_status_text.setText(
            f"{speed_text} / {active_count} active / {file_pct}%"
        )
        self.lbl_status.setText(
            f"{self.lbl_status.text()} • Current file {file_pct}%"
        )

    def _refresh_aggregate_progress(self):
        total_bytes = sum(
            max(0, int(metadata.get("size", 0) or 0))
            for metadata in self.files_metadata
        )
        downloaded_bytes = 0
        for metadata in self.files_metadata:
            ...
            if metadata.get("completed"):
                ...
                continue
            transfer = self.file_transfer_stats.get(metadata["id"])
            if transfer:
                current = max(0, int(transfer.get("current", 0) or 0))
                downloaded_bytes += min(current, size) if size else current

        if total_bytes > 0:
            ratio = min(1.0, downloaded_bytes / total_bytes)
            self.batch_progress_bar.setMaximum(1000)
            self.batch_progress_bar.setValue(int(ratio * 1000))
            self.lbl_status.setText(
                f"Downloaded {self.completed}/{self.total_items} files • "
                f"{self._format_bytes(downloaded_bytes)} / "
                f"{self._format_bytes(total_bytes)} ({ratio * 100:.1f}%)"
            )
        else:
            ratio = (
            ...
            self.batch_progress_bar.setMaximum(1000)
            self.batch_progress_bar.setValue(int(min(1.0, ratio) * 1000))
            self.lbl_status.setText(
                f"Downloaded {self.completed} out of {self.total_items} items"
            )
    ...
    def run_health_check(self):
        ...
        if corrupt > 0 or missing > 0:
            ...
            self.btn_verify.setText("Review files")
        else:
        ...
        self.btn_verify.setText("Check files")
    ...
    def set_reselect_loading(self, loading):
        if loading:
            ...
            self.btn_reselect.setText("Change files")
